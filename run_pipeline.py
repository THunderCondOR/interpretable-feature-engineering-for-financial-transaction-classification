...
Usage:
    ...
    python run_pipeline.py --config configs/gender.yaml --steps stats,prompts,cot,claims
"""
...
def run_prompts(config: dict) -> None:
    ...
    summary_str  = (out_dir / config["output"]["summary_stats"]).read_text(encoding="utf-8")
    ...
    with open(clients_path, "w", encoding="utf-8") as f:
        for rec in records:
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")
    print(f"Saved {len(records)} prompts → {clients_path}")
...
def run_cluster(config: dict) -> None:
    ...
    # Load claims, flatten to two parallel lists
    records = []
    ...
    # Flat lists: one entry per claim
    flat_claims = []
    flat_labels = []
    flat_cids   = []
    for rec in records:
    ...
    # Text cleaning — keep only non-empty Russian claims
    ...
    embeddings = embed_texts(flat_claims)

    # Remove label-inconsistent outliers
    ...
    # Filter clusters by class-label separability
    ...
    # Build per-client feature vectors (count of claims per cluster)
...
STEPS    = ["stats", "prompts", "cot", "claims", "cluster", "lora"]
