...
echo "Waiting for gpt_oss_rosbank_cot_repair to finish"
while tmux has-session -t gpt_oss_rosbank_cot_repair 2>/dev/null; do
  sleep 30
done

for pass in 1 2; do
  echo "Extra rosbank CoT resume pass $pass/2"
...
echo "Backing up old rosbank claims before full rebuild"
...
echo "GPT-OSS post-rosbank repair completed."
